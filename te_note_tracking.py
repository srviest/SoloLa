...
def tent(melody, min_pitch=30.0, max_cont_diff=0.8, max_cand_diff=3.5, min_melo_len=18, min_pattern_length=8):
    if melody.length == 0:
    ...
    for idx, subm in enumerate(submelo_list):
        tr = get_trend(subm, min_pattern_length=min_pattern_length)
        nt = get_notes(subm, tr, min_pattern_length=min_pattern_length)
        if idx + 1 in melody_cand_dict.keys():
            sign, sub_idx = melody_cand_dict[idx + 1]
            seg_pos = max(0, sub_idx - pm.MC_LENGTH/2 - nt[-1].onset)
            seg = Segment(sign, seg_pos, pm.MC_LENGTH, melody)
            if isinstance(nt[-1], CandidateNote):
                nt[-1].segs.append(seg)
            else:
                nt[-1] = CandidateNote(segs=[seg], note=nt[-1])
...
def get_trend(melody, min_vib_amp=0.4, min_pattern_length=8, min_cs_amp=0.8, max_cs_amp=3.0, max_cs_length=33):
